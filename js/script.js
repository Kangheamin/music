...
const addShow = (index, articleLists) => {
    for(let v of articleLists){ //8번이 돌아가
...
const initial = ()=>{
    for(let v of audio){
        v.pause();
        v.load(); //멈추고 처음으로 돌아가게 한다
        v.closest("article").querySelector(".photo").classList.remove("on");
    }
};
...
prevBtn.addEventListener("click", () => {
    ...
    //이전에 접근되어지는 인덱스 번호계산하여 인덱스번호를 전달하고자 구조 구성
    //active = 0 -> 7 -> 6 -> 5 -> ... 4 ...
    if(active == 0){
        active = len;
      }else{
          active--; //active = 0 -> 7 -> 5 ... -> 0❌= 7
      }
...
nextBtn.addEventListener("click", () => {
    ...
    num--;
    frame.style.transform = `rotate(${angle * num}deg) translate(-50%, -50%)`;
    //다음에 접근되어지는 인덱스 번호를 계산하여 인덱스번호를 전달하고자 구조를 구성
    if(active == len){
      active = 0;
    }else{
        active++; //active = 0 -> 1 -> 2 .... 7 -> 8❌= 0
    }
    addShow(active, articleLists);
})
